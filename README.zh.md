...
> **核心 vs. SaaS 层。** 本仓库是**纯粹、MIT、与细分领域无关的 AI 大脑** — 刻意*不含*计费、认证或客户账户（`tenant_id` 是命名空间，不是客户）。任何产品化（认证、计费、面板、API 网关、按客户计量）都属于一个**调用本引擎 HTTP API 的独立层**，并把每个*客户 → tenant_id 命名空间*做映射。大脑保持可 fork、可学习；商业外壳绝不渗入其中。
...
| **5. 智能体编排器** | CORE | LangGraph 主管–工作者（研究员 → 创作者 → **评论者**）· **评论助手** 端到端 · **人工介入审核** · 领域路由器 · **结构化/受约束工具输出** · **意图分流** · **多轮记忆** · **拒答（abstention）** | 多智能体设计、依据与反幻觉、HITL 工作流、领域路由 | ⏳ 计划中 |
| **5.5 安全与护栏** | CORE | **提示注入防御** · **PII 脱敏** · 进/出**毒性审核** · **红队测试** · 输出护栏框架 · 优雅降级 · 成本/速率护栏 | 加固一个摄入不可信用户评论的 LLM | ⏳ 计划中 |
| **6. 生产、MLOps & 评估** | CORE | 完整 Docker 栈 · 监控/日志（LangFuse、Prometheus + Grafana）· **数据生命周期**（向量 CRUD/删除/同步、去重、**嵌入迁移**）· **规模化评估**（在线评估、黄金/回归集、prompt 版本管理、LLM 评判校准）· CI/CD 再训练 · 实验跟踪（W&B / MLflow）· 版本管理（DVC / HF Hub） | 可观测性、可复现 ML、随时间保持知识库正确、重型 MLOps | ⏳ 计划中 |
...
| **MMR**（检索多样性） | 避免 top-k 全是近重复块 | 最大边际相关（maximal marginal relevance）；覆盖一个查询的更多侧面 |
| **时效感知（temporal / freshness-aware）检索** | 权衡新鲜度，而非只看相关性 — 过时块即垃圾，哪怕主题相关 | 时间衰减打分；按领域的 flag（金融/新闻开，稳定知识关） |
| **上下文窗口预算** | 排序并裁剪上下文以塞进窗口 | "lost in the middle" 问题；空间不够时该保留什么 |
