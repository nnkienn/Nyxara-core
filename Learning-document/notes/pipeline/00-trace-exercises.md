...
**1a.** ✅ *ĐÃ TRẢ LỜI.* `recursive_chunker.py` có 2 hàm: `split_by_separators` nhận `separators`,
`recursive_chunk` chỉ tính trên `start`/`end`. `ingest.py` import `recursive_chunk`.
→ **`POST /ingest` đang chạy Fixed-Size Chunking, KHÔNG phải Recursive.** Cái tên nói dối.
→ Bug loại *"tên hàm ≠ hành vi"*: không bao giờ crash, chỉ âm thầm làm mọi kết luận sau sai.
Hậu quả: benchmark "Recursive vs Semantic" ở Phase 3 sẽ so **sai baseline**.

**1b.** ✅ *ĐÃ TRẢ LỜI.* `grep -rn "split_by_separators" app/ tests/` → chỉ có chính nó (đệ quy)
và 1 test. Không dòng nào trong `app/` gọi.
...
|---|---|
| `new_doc` | ? |
| `to_upsert` / `to_skip` / `to_delete` | ? |

*Lần ingest 2:* sửa chunk giữa → `chunks = ["mèo", "vịt", "gà"]`

| | giá trị |
|---|---|
| `old_doc` | ? (= `new_doc` lần 1) |
| `new_doc` | ? |
| `to_upsert` / `to_skip` / `to_delete` | ? |
